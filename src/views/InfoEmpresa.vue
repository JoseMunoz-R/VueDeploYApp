<template>
  ...
  >
    ...
    <b-button class="mt-3 mr-2" variant="info" @click="postularCandidato({emailem: perfilEmpresa.email, idvacante: item.id}); getPostulantes({emailem: perfilEmpresa.email, idvacante: item.id})">Postularme</b-button>
    <b-button class="mt-3" variant="warning" @click="declinarPostulacion({emailem: perfilEmpresa.email, idvacante: item.id})">Declinar</b-button>
...
export default {
    ...
    methods: {
        ...mapActions(['getInfoEmpresa', 'getInfoVacantes', 'postularCandidato', 'declinarPostulacion', 'getPostulantes'])
    },
    computed: {
        ...mapState(['perfilEmpresa', 'vacantes', 'info', 'vacantePostulante'])
    },
    created() {
